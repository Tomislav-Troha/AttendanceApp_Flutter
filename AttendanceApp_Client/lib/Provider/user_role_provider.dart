...
class UserRoleProvider extends GetxController{
  ...
  Future<ServerResponse> getUserRoles() async {
    ...
    if(serverResponse.isSuccessful){
      if(serverResponse.result is List<dynamic>){
        List<UserRoleResponseModel> responseModels = (serverResponse.result as List<dynamic>)
            .map((item) => UserRoleResponseModel.fromJson(item as Map<String, dynamic>))
            .toList();

        serverResponse.result = responseModels;
      }
    } else {
      serverResponse.error = "Error while getting userRoles ${serverResponse.error}";
    }
    return serverResponse;
  }

  Future<ServerResponse> addUserRole (ContractRequestModel model, int userId) async {
    ...
    if(serverResponse.isSuccessful){
      ...
      serverResponse.error = "Error while adding contract ${server.reasonPhrase}";
    }
    return serverResponse;
  }
}