...
class TrainingDateProvider extends ChangeNotifier{
    ...
    if (serverResponse.isSuccessful) {
        if (serverResponse.result is List<dynamic>) {
          ...
          (serverResponse.result as List<dynamic>)
              .map((item) => TrainingDateResponseModel.fromJson(item as Map<String, dynamic>))
              .toList();
      ...
      serverResponse.error = "Error while getting training date ${serverResponse.error}";
    }
  ...
  Future<ServerResponse> deleteTrainingDate(int id) async {
    ...
    if(serverResponse.isSuccessful){
      ...
      if(serverResponse.error!.contains("error: 23503")){
        ...
        serverResponse.error = "Error while deleting training date ${serverResponse.error}";
      }
    }
    return serverResponse;
  }
  ...
  Future<ServerResponse> getTrainingDateForEmployee(DateTime? currentDate) async {

    var url = currentDate == null ? 'trainingDate/getTrainingDateForEmployee' : 'trainingDate/getTrainingDateForEmployee/$currentDate';
    ...
    if (serverResponse.isSuccessful) {
      ...
      serverResponse.error = "Error while getting training date for employee ${response.reasonPhrase}";
    }
    return serverResponse;
  }
  ...
  Future<ServerResponse> addTrainingDate (TrainingDateRequestModel model) async {
    ...
    if(serverResponse.isSuccessful){
    TrainingDateResponseModel responseModels = TrainingDateResponseModel.fromJson(serverResponse.result);
    serverResponse.result = responseModels;
    }
    else {
      serverResponse.error = "Error while adding training date ${serverResponse.error}";
    }
    return serverResponse;
  }
}