...
import '../../Widget-Helpers/custom_text_button.dart';
import '../../Widget-Helpers/custom_text_form_field.dart';
import '../../Widget-Helpers/show_multi_items.dart';
...
class _AddTrainingDate extends State<AddTrainingDate> {
  ...
  void initAllTrainings() async {
    ...
    if (userByMember.isSuccessful) {
      userName = userByMember.result.cast<UserResponseModel>() as List<UserResponseModel>;
    }
    else{
  ...
  Widget build(BuildContext context) {
    ...
    return SizedBox(
      ...
      children: [
              ...
              if(!future.hasData){
                ...
                return Padding(
                    ...
                    child: CustomDropdownButton(
                        ...
                        items: trainings.map<DropdownMenuItem<TrainingResponseModel>>((TrainingResponseModel? values) {
                        ...
                        onChanged: (value) {
                          setState(() {
                            ...
                            trainingDateController.trainingID.text = _valueTraining!.ID_training.toString();
                          });
                        }, hint: 'Training'
                    )
                );
        ...
        Wrap(
          children: _valueMember!.map((e) => Chip(
        ...
        text: "Spremi",
        onPressed: () async {
          String? errorMessage = trainingDateController.validateTrainingDateInputs(
          ...
          if(errorMessage != null){
            AppMessage.showErrorMessage(message: errorMessage, duration: 1);
          }
          else if(_valueMember!.isEmpty){
            AppMessage.showErrorMessage(message: "Odaberi članove", duration: 1);
          }
          else {
            trainingDateController.requestModel.trainingModel?.iD_training = _valueTraining?.ID_training;
            trainingDateController.requestModel.dates = trainingDateController.parseDate(trainingDateController.date.text);
            trainingDateController.requestModel.timeFrom = trainingDateController.parseTime(trainingDateController.timeFrom.text);
            trainingDateController.requestModel.timeTo = trainingDateController.parseTime(trainingDateController.timeTo.text);
            ...
            ServerResponse response = await trainingDateProvider.addTrainingDate(trainingDateController.requestModel);
            if(response.isSuccessful){
              AppMessage.showSuccessMessage(message: "Successfully added training date");
              setState(() {
