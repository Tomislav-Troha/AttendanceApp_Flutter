...
class _EmployeeAdmin extends State<EmployeeAdmin>{
  ...
  void getAllUsersForEmployee() async {
    ServerResponse allUsersEmployee = await employeeAdminProvider.getUserEmployee();
          ...
          child: FutureBuilder(
            ...
            builder: (context, future){
              if(!future.hasData) {
                ...
                return const Text("There is no employees yet", textScaleFactor: 1.6,);
              }
              else{
                List<UserResponseModel>? list = future.data?.result.cast<UserResponseModel>();
                              ...
                              return CustomDialog(
                                ...
                                children: [
                                  TextButton(
                                    ...
                                    onPressed: () {
                                      if(token["UserRoleId"] == EnumUserRole.Admin || token["UserRoleId"] == EnumUserRole.Moderator){
                                        Navigator.pop(context);
                                        Navigator.of(context).push(MaterialPageRoute(
                                            builder: (context) {
                                              return EditMemberAdmin(userID: list[index].userId!);
                                            }));
                                      }
                                      else {
                                        Navigator.pop(context);
                                        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content:
                                        Text("Only admin or moderator can edit employees", textAlign: TextAlign.center,),
                                  ...
                                  TextButton(
                                    ...
                                    onPressed: () async {
                                      if(token["UserRoleId"] == EnumUserRole.Admin || token["UserRoleId"] == EnumUserRole.Moderator){
                                        ...
                                        AppMessage.showErrorMessage(message: "Only admin or moderator can delete employees");
                                      }
                                    },
                                  )
                                ],
                          ...
                          child: Padding(
                            padding: const EdgeInsets.all(2.0),
                             child: ListTile(
                              ...
                              subtitle: Text(list[index].userRoleModel!.roleDesc == null ? "Role not set" : list[index].userRoleModel!.roleDesc!),
                          ),
                        )
                    ));
                  },
