...
import 'package:swimming_app_client/Widget-Helpers/training_time_utils.dart';
...
class SumbitAttendance extends StatefulWidget {
  ...
  int index;
  SumbitAttendance({super.key, required this.trainingDateResponse, this.attendanceResponse, required this.index});
...
class _SubmitAttendance extends State<SumbitAttendance> {
  ...
  late final lateTime;
   ...
   void initialize() async {
     ...
     ServerResponse getAttendanceByUser = await attendanceProvider.getAttendance();
     if(getAttendanceByUser.isSuccessful){
       ...
       for(var element in attendances!){
         if(element.trainingDateModel!.iD_TrainingDate == widget.trainingDateResponse[widget.index].iD_TrainingDate){
           if(element.attDesc == AttendanceDesc.Accepted || element.attDesc == AttendanceDesc.AcceptedWithLate){
     ...
     ServerResponse allTrainingDatesByUser = await trainingDateProvider.getTrainingDate(null);
  ...
  void initState() {
    ...
    if(TrainingTimeUtils.isTrainingInProgress(widget.trainingDateResponse[widget.index].dates!.toLocal(),
        TimeOfDay.fromDateTime(widget.trainingDateResponse[widget.index].timeFrom!.toLocal()),
        TimeOfDay.fromDateTime(widget.trainingDateResponse[widget.index].timeTo!.toLocal()))) {
      _isInTime = true;
    }
    else {
      _isInTime = false;
    ...
    lateTime = TrainingTimeUtils.calculateLateTime(widget.trainingDateResponse[widget.index].dates!.toLocal(),
        TimeOfDay.fromDateTime(widget.trainingDateResponse[widget.index].timeFrom!.toLocal()),
        TimeOfDay.fromDateTime(widget.trainingDateResponse[widget.index].timeTo!.toLocal()));
  ...
  void addAttendanceNotChecked(){
      ...
      attendanceProvider.addAttendance(controller.requestModel).then((value) =>
      {
        _isAttendanceCompleted = true
      }).catchError((error) {
  ...
  Widget build(BuildContext context) {
    ...
    DateTime _formattedDates = widget.trainingDateResponse[widget.index].dates!.toLocal();
    String formattedDates = _formattedDates.toIso8601String().substring(0, 10);

    DateTime _dateNow = DateTime.now();
    String dateNow = _dateNow.toString().split(' ')[0];

    DateTime timeFrom = widget.trainingDateResponse[widget.index].timeFrom!.toLocal();
    DateTime timeTo = widget.trainingDateResponse[widget.index].timeTo!.toLocal();
                    ...
                    Column(
                      children: [
                        Padding(
                            ...
                            child: Text(
                              widget.trainingDateResponse[widget.index].trainingModel!
                                  .trainingType!,
                              textScaleFactor: 1.6,
                            ))
                      ],
                    ...
                    Column(
                      children: [
                        Padding(
                            ...
                            child: Visibility(
                              visible: _isInTime,
                              child: _isAttendanceCompleted ? Container()
                                  : AnimatedButton(
                                ...
                                onPress: () {
                                    if (_isInTime) {
                                      if(_userIsLateForAttendance) {
                                        controller.requestModel.attDesc = AttendanceDesc.AcceptedWithLate;
                                        controller.requestModel.type = "Kašnjenje: $lateHours sati i $lateMinutes minuta";
                                      } else {
                                        controller.requestModel.attDesc = AttendanceDesc.Accepted;
                                        controller.requestModel.type = "Na vrijeme";
                                      }
                                      controller.requestModel.trainingModel?.iD_training = widget.trainingDateResponse[widget.index].trainingModel?.ID_training;
                                      controller.requestModel.userModel?.userId = widget.trainingDateResponse[widget.index].userModel?.userId;
                                      controller.requestModel.trainingDateModel?.iD_TrainingDate = widget.trainingDateResponse[widget.index].iD_TrainingDate;
                                      List<UserRequestModel> userList = [];
                                      userList.add(UserRequestModel(
                                          surname: "",
                                          password: "",
                                      ...
                                      AppMessage.showErrorMessage(message: "Nije u vremenu");
                                    }
                        ...
                        Visibility(
                          child: Padding(
                            ...
                            Column(
                              ...
                              children: <Widget> [
                                !_isAttendanceCompleted ? AttendanceStatusWidget(
                                  index: widget.index,
