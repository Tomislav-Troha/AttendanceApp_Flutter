...
class _AttendanceEmployee extends State<AttendanceEmployee> {
  ...
  void initializeAttendanceAndMembers() async{
    ...
    ServerResponse allAttendances = await attendanceProvider.getAttendanceAll(userID);
    if(allAttendances.isSuccessful){
      attendances = allAttendances.result.cast<AttendanceResponseModel>();
      for (var att in attendances) {attendList?.add(att);}
    }
    else {
    ...
    if(members.isSuccessful){
      ...
      for (var att in allMembers) {memberList?.add(att);}
    }
    else {
  ...
  void initializeTrainingDates() async {
    ServerResponse allTrainingsForEmployees = await trainingDateProvider.getTrainingDateForEmployee(_currentDate);
    if(allTrainingsForEmployees.isSuccessful){
      trainingsForEmployees = allTrainingsForEmployees.result.cast<TrainingDateResponseModel>();
    }
    else {
                              ...
                              child: CustomTextFormField(
                                  ...
                                  onTap: () => {
                                    ShowMultiItems.showMultiMembers(
                                        context, memberList!, (selectedList) {
                                      setState(() {
                                            ...
                                            child: Container(
                                              ...
                                              child: TextButton(
                                                ...
                                                onPressed: () async {
                                                  ...
                                                  await trainingDateProvider.deleteTrainingDate(filteredList[index].iD_TrainingDate!);
                                    ...
                                    return StatefulBuilder(
                                      builder: (BuildContext context, StateSetter setState) {
                                        return CustomDialog(
                                          ...
                                          children: [
                                            if (attendanceDoesNotExist(index, attendList, filteredList)) ...{
                                              DropdownButton<String>(
                                                ...
                                                items: <String>[AttendanceDesc.Late, AttendanceDesc.Sick]
                                              ...
                                              TextButton(
                                                child: const Text("Ok"),
                                                  onPressed: () {
                                                    ...
                                                    attendanceRequestModel.type = "Ne bitno";
                                                    attendanceRequestModel.trainingModel?.iD_training = filteredList[index].trainingModel?.ID_training;
                                                    attendanceRequestModel.userModel?.userId = filteredList[index].userModel?.userId;
                                                    attendanceRequestModel.trainingDateModel?.iD_TrainingDate = filteredList[index].iD_TrainingDate;
                                                    ...
                                                    attendanceRequestModel.trainingDateModel?.userModelList = userList;

                                                    attendanceProvider.addAttendanceNotSubmitted(attendanceRequestModel).then((value) => {
                                                      AppMessage.showSuccessMessage(message: "Uspješno zabilježeno"),
                                                      Navigator.pop(context)
                                                    }).catchError((error) => {
                                                      Navigator.pop(context),
