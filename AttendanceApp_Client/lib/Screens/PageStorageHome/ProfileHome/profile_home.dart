...
import '../../../Widget-Helpers/custom_expansion_tile.dart';
import '../../../Widget-Helpers/sensitive_info_toggle.dart';
...
class _ProfileState extends State<Profile>{
  ...
  void getUserData() async {
    ServerResponse serverResponse = await userProvider.getUserByID(int.parse(token["UserID"]));
  ...
  void initializeUserModel() async {
    ...
    if (cachedUser == null) {
      ...
      if(users.isSuccessful){
        ...
        AppMessage.showErrorMessage(message: "Error loading user data ${users.error}");
      }
    }
    else {
  ...
  Future insertImage(ImageSource media) async {
    ...
    ServerResponse response = await userProvider.setProfileImage(userRequestModel, int.parse(token["UserID"]));
    if(response.isSuccessful){
      AppMessage.showSuccessMessage(message: "Image successfully uploaded");
    } else {
      AppMessage.showErrorMessage(message: "Error uploading image ${response.error}");
    }
  ...
  Future<ContractResponseModel?> getContractByUserId(int id) async {
    ...
    if (contractByUserId.isSuccessful) {
      ...
      AppMessage.showErrorMessage(message: "Error loading contract data ${contractByUserId.error}");
      return null;
    }
  }
                                 ...
                                 child: Column(
                                   children: [
                                     FutureBuilder<ContractResponseModel?>(
                                       future: getContractByUserId(int.parse(token["UserID"])), // Replace with your Future function
                                       builder: (BuildContext context, AsyncSnapshot<ContractResponseModel?> snapshot) {
                                         if (snapshot.connectionState == ConnectionState.waiting) {
                                           ...
                                           ContractResponseModel contract = snapshot.data!;
                                           return Column(
                                             children: [
                                               CustomExpansionTile(
                                                 title: Column(
                                                 ...
                                                 children: [
                                                   ListTile(
                                                     ...
                                                     subtitle: Text(
                                                       contract.contractTypeModel?.contractTypeName ?? contract.userRoleModel!.roleName!,
                                                       style: const TextStyle(fontSize: 16, fontWeight: FontWeight.bold),
                                                     ),
                                                   ),
                                                   if (userModel?.userRoleModel?.roleID != int.parse(EnumUserRole.Member))
                                                     ...[
                                                   ListTile(
                                                     title: Text(
                                                       "Salary Package",
                                                     ...
                                                     subtitle: SensitiveInfoToggle(
                                                       sensitiveInfo: contract.salaryPackageTypeModel?.salaryPackageName ?? '',
                                                     ),
                                                   ),
                                                   ListTile(
                                                     title: Text(
                                                     ...
                                                     subtitle: Text(
                                                       contract.jobRoleModel?.jobRoleName ?? '',
                                                       style: const TextStyle(fontSize: 16, fontWeight: FontWeight.bold),
                                                     ),
                                                   ),
                                                   ListTile(
                                                     ...
                                                     subtitle: Text(
                                                       DateFormat('dd.MM.yyyy').format(contract.startDate!.toLocal()),
                                                       style: const TextStyle(fontSize: 16, fontWeight: FontWeight.bold),
                                                     ),
                                                   ),
                                                   ListTile(
                                                     ...
                                                     subtitle: Text(
                                                       DateFormat('dd.MM.yyyy').format(contract.expiryDate!.toLocal()),
                                                       style: const TextStyle(fontSize: 16, fontWeight: FontWeight.bold),
                                                     ),
                                                   ),
                                         ],
                                           ...
                                           setState(() {
                                             showDialog(context: context, builder: (context){
                                               return CustomDialog(
                                                 ...
                                                 children: [
                                                   TextButton(
                                                       child:const Text("Get image from gallery"),
                                                       onPressed:(){
                                                         Navigator.of(context).pop();
                                                         insertImage(ImageSource.gallery);
                                                       }),

                                                   TextButton(
                                                       child:const Text("Get image from camera"),
                                                       onPressed:(){
                                                         Navigator.of(context).pop();
                                                         insertImage(ImageSource.camera);
                                                       })
                                                 ],
                                               );
                                             });
                                         ...
                                         child: ClipRRect(
                                           ...
                                           child: SizedBox(
                                             ...
                                             Container(
                                               ...
                                               child: const Icon(Icons.photo_camera_front_rounded, size: 100),
                                             ),
                                           ),
                                         ),
                                       ),
                               ...
                               child: Column(
                                 children: [
                                     ...
                                     child:  ElevatedButton(
                                       ...
                                       onPressed: () {
                                         Navigator.of(context).push(MaterialPageRoute(
                                             builder: (context) {
                                               return EditProfile(userCache: userCache, callback: getUserData);
                                             }));
                                       },
                                     ),
                                   ),
                                   Visibility(
                                     ...
                                     child: Padding(padding: const EdgeInsets.symmetric(vertical: defaultPadding, horizontal: defaultPadding),
                                       child:  ElevatedButton(
                                     ...
                                     child:  ElevatedButton(
                                       ...
                                       onPressed: () {
                                         showDialog(context: context,
                                             builder: (context){
                                               controller.changePasswordEmail.text = userModel!.email!;
                                                       ...
                                                       children: [
                                                         Form(
                                                           ...
                                                           child: Column(
                                                             children: [
                                                               TextFormField(
                                                                 controller: controller.changePasswordEmail,
                                                                 validator: (value) {
                                                                   if (controller.changePasswordEmail.text.isEmpty) {
                                                               ...
                                                               TextFormField(
                                                                 ...
                                                                 validator: (value) {
                                                                   ...
                                                                   if(controller.apiErrors != null){
                                                                     return controller.apiErrors;
                                                                   }
                                                                   return null;
                                                                 },
                                                                 // cursorColor: kPrimaryColor,
                                                               ...
                                                               TextButton(
                                                                   ...
                                                                   onPressed: () async {
                                                                     ...
                                                                     controller.clearApiErrorMessage();

                                                                     if (formKey1.currentState!.validate()) {
                                                                       controller.changePasswordRequestModel.email = controller.changePasswordEmail.text;
                                                                       controller.changePasswordRequestModel.password = controller.newPassword.text;

                                                                       var dateProviderLogin = await userProvider.changePassword(controller.changePasswordRequestModel);
